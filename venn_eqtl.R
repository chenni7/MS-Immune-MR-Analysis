...
data1=read.table("eQTL.txt",header=F,sep = "\t")
...
venn.plot <- venn.diagram(
  ...
  category.names = c("eQTL", "Palmitoylation"),
...
pdf("venn_eqtl.pdf",12,12);
grid.draw(venn.plot);
dev.off()
mylist= list(Set1 = dfset1, Set2 = dfset2)
vedata=Reduce(intersect,mylist)
write.table(vedata,"Venn_eqtl.txt",sep="\t",quote=F,col.names=F,row.names=F)
